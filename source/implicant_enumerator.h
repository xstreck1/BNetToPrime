...
class ImplicantEnumerator {
	...
	static void nextLevel(const size_t var_count, const size_t depth, DNF & source, DNF & result) {
		...
		for (DNF::const_iterator it = source.cbegin(); it != source.cend(); it++) {
			cout << "\rDistributing. Depth " << depth << ". Value " << distance(source.cbegin(), it) << "/" << source.size();
		...
		for (vector<vector<DNF> >::const_iterator dc_level_it = distribution.cbegin(); dc_level_it < distribution.cend(); dc_level_it++) {
			for (vector<DNF>::const_iterator one_level_it = dc_level_it->cbegin(); one_level_it + 1 < dc_level_it->cend(); one_level_it++) {
				cout << "\rMerging. Depth " << depth << ". Box " << distance(distribution.cbegin(), dc_level_it) << "/" << distribution.size();
		...
		source.resize(distance(merged.begin(), new_end));
		copy(merged.begin(), new_end, source.begin());
	}
...
public:
	...
	static DNF compactize(const size_t var_count, DNF merged) {
		...
		// Make compact
		for (size_t depth = 0; depth < var_count && !merged.empty(); depth++) {
